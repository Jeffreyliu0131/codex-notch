...
For eligible new explicit-approval signals, the notch expands for eight seconds. A silent system-notification request is submitted only after checking permission, foreground state, source freshness and whether the approval signal is still current. Its content contains the device label and task title; its hidden payload contains only host and thread identifiers, excluding prompt text and workspace paths. System acceptance is not proof of delivery or reading. While permission is unresolved, a transient in-memory queue keeps distinct task signals and drops entries older than two minutes when the queue is drained before submission; the app does not persist that queue or lifecycle telemetry. macOS controls the retention of submitted system notifications.
